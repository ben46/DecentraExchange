...
contract CookTarget {
    function onCook(address sender, bytes calldata data)
        public
        payable
        virtual
...
contract BoringCooker is Domain {
    ...
    bytes32 private constant COOK_SIGNATURE_HASH =
        0x22efff3742eba32ab114c316a3e6dae791aea24d5d74f889a8f67bc7d4054f24;
        ...
        )
    {
        (
            ...
            bytes32 s
                ...
                data,
                (address, CookTarget, bytes, uint256, uint256, uint8, bytes32, bytes32)
            );
        ...
        require(
            ecrecover(
                _getDigest(
                    keccak256(
                        abi.encode(COOK_SIGNATURE_HASH, data_, sender, nonces[sender]++, deadline)
                    )
                ),
                v,
                r,
    ...
    function cook(uint8[] calldata actions, bytes[] calldata datas) external payable {
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_CALL) {
                ...
                (address target, bytes4 signature, bytes memory data, uint256 value) = abi.decode(
                    datas[i],
                    (address, bytes4, bytes, uint256)
                );
                require(signature != CookTarget.onCook.selector, "Use action cook");
                (bool success, bytes memory localResult) = target.call{ value: value }(
                    abi.encodePacked(signature, data)
                );
                if (!success) {
                ...
                (CookTarget target, bytes memory data, uint256 value) = abi.decode(
                    datas[i],
                    (CookTarget, bytes, uint256)
                );
                (bool success, bytes memory localResult) = target.onCook{ value: value }(
                    msg.sender,
                    data
                );
                if (!success) {
